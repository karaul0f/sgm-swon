...
public class NextButtonHandler : MonoBehaviour
{
    ...
    void UpdateButton()
    {
        ...
        if (m_tourManagerScript.CurrentBlockIndex == (int) m_tourManagerScript.TourConfigurator.TourCompleteStatus &&
            m_tourManagerScript.TourConfigurator.TourCompleteStatus == ETourStatus.Final)
            m_textOnButton.text = "Создать тур!";
        else
            m_textOnButton.text = "Далее";
